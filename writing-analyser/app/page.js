...
function ScoreBadge({ score }) {
  return (
    <span style={{
      ...
      background: scoreColors[score] || "#999",
...
function CCSSCell({ assessment }) {
  if (!assessment) return <td className="p-4 align-top border-b border-stone-200">—</td>;
  const { standards_met = [], areas_for_growth = [], ccss_level } = assessment;
  return (
    <td className="p-4 align-top border-b border-stone-200" style={{ minWidth: 240 }}>
      <span style={{
      ...
      {standards_met.length > 0 && (
        <div className="mb-3">
          <div style={{ fontSize: 10, fontWeight: 700, color: "#2D5016", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>
            Standards Met
          </div>
      ...
      {areas_for_growth.length > 0 && (
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#B05A00", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>
            Areas for Growth
          </div>
...
function downloadCSV(students) {
  ...
  a.href = url;
  a.download = "student_writing_analysis.csv";
  a.click();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F3EE", padding: "40px 24px", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        {loading && (
          <div style={{
            background: "#EAF2DC", border: "1px solid #5A8A2A", borderRadius: 8,
            padding: "20px 24px", display: "flex", alignItems: "center", gap: 14, marginBottom: 24,
          }}>
            <div style={{
              width: 20, height: 20, border: "2px solid #5A8A2A", borderTopColor: "transparent",
              borderRadius: "50%", animation: "spin 0.8s linear infinite",
            }} />
            <span style={{ color: "#2D5016", fontWeight: 600 }}>
              Analysing handwriting and assessing against CCSS ELA Grades 6–8…
            </span>
          </div>
        )}
        ...
        {error && (
          <div style={{
            background: "#FDF0EF", border: "1px solid #C0392B", borderRadius: 8,
            padding: "16px 20px", color: "#C0392B", marginBottom: 24, fontSize: 14,
          }}>
            ⚠ {error}
          </div>
        )}
        ...
        {students.length > 0 && (
          <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E0D8CE", overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
            <div style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #E0D8CE" }}>
              <div>
                <span style={{ fontFamily: "Georgia, serif", fontSize: 18, color: "#1A1A1A" }}>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      ["Spelling", "8%"],
                      ["Observations", "20%"],
                      ["CCSS ELA Assessment · Grades 6–8", "38%"],
                    ].map(([label, width], i) => (
                      <th key={label} style={{
                        padding: "12px 16px", textAlign: i === 2 ? "center" : "left",
                        fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase",
                        color: i === 4 ? "#1A3A5C" : "#2D5016",
                        borderBottom: `2px solid ${i === 4 ? "#1A3A5C" : "#2D5016"}`,
                        background: i === 4 ? "#E8F0F8" : "#EAF2DC",
                ...
                <tbody>
                  {students.map((s, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#FAFAF7" }}>
                      <td style={{ padding: "14px 16px", verticalAlign: "top", borderBottom: "1px solid #E0D8CE", fontSize: 14 }}>{s.student_name}</td>
                      <td style={{ padding: "14px 16px", verticalAlign: "top", borderBottom: "1px solid #E0D8CE", fontSize: 13, color: "#6B6B6B", lineHeight: 1.7, fontFamily: "Georgia, serif", minWidth: 180 }}>{s.written_text}</td>
                      <td style={{ padding: "14px 16px", verticalAlign: "top", borderBottom: "1px solid #E0D8CE", textAlign: "center" }}>
                        <ScoreBadge score={s.spelling_score} />
                        <div style={{ fontSize: 11, color: "#6B6B6B", marginTop: 4 }}>/5</div>
                      </td>
                      <td style={{ padding: "14px 16px", verticalAlign: "top", borderBottom: "1px solid #E0D8CE", fontSize: 13, color: "#6B6B6B", lineHeight: 1.6, minWidth: 160 }}>{s.observations}</td>
